...
// ---------------------------------------------------------------------------
// ジャストミート(IMPROVEMENTS §6.1.1)— 案B: 自動打球は維持しつつ、接触の瞬間に
// ショットキーをもう一度タップ(チャージキーは保持したまま別キーを叩く)すると
// 「ジャストミート」になり威力・狙い・回転に控えめなボーナス。外すと通常打球(ペナルティなし)。
// 窓は接触予定の直前。難球(速球・走らされ=低品質)ほど窓を狭くする。
// ---------------------------------------------------------------------------
/** ジャスト窓の基準幅(秒)。自動打球の直前この時間内にタップがあれば成立 */
export const JUST_WINDOW_BASE = 0.13
/** 難球での窓下限(秒)。hardness=1 でこの幅まで狭まる */
export const JUST_WINDOW_MIN = 0.06
/** 窓の難度: incomingSpeed をこの範囲で 0..1 に正規化(速いほど狭い) */
export const JUST_HARD_SPEED_LO = 14
export const JUST_HARD_SPEED_HI = 30
/** 窓の難度: この品質を下回ると(走らされ等)さらに狭くする基準 */
export const JUST_QUAL_REF = 0.55
